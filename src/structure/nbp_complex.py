...
class ComplEx(NeuralBinaryPredicate, nn.Module):
    ...
    def get_relation_emb(self, relation_id_or_tensor, inv=False):
        ...
        if isinstance(relation_id_or_tensor, torch.Tensor):
            rel_id = relation_id_or_tensor.clone().detach().to(self.device)
        else:
            rel_id = torch.tensor(relation_id_or_tensor, device=self.device)
        if inv:
    ...
    def get_entity_emb(self, entity_id_or_tensor):
        ...
        if isinstance(entity_id_or_tensor, torch.Tensor):
            ent_id = entity_id_or_tensor.clone().detach().to(self.device)
        else:
            ent_id = torch.tensor(entity_id_or_tensor, device=self.device)
